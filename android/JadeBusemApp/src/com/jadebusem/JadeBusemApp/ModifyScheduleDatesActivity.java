...
/*####################################################################################################
# ModifyScheduleDatesActivity()
# Main class in Activity
####################################################################################################*/
public class ModifyScheduleDatesActivity extends Activity {
...
# Return: None
# Method that initialize all needed stuff to set Time in existing schedule
####################################################################################################*/
    @Override
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(schedule_temp!=null)
        {
            ...
            new_schedule = (Schedule) schedule_temp.get("new_schedule");
        }
        ...
        ArrayList<ScheduleDate> dates = schedule.getScheduleDates();
        for(int i=0;i<dates.size();i++)
        {
            ScheduleDate record = dates.get(i);
            int tabIndex = record.toIntFromEnum(record.getDay());
            timeList[tabIndex].add(record.getTime());
        }
...
# Inputs: View view
# Return: None
# Action of saveTimetableButton -> add schedule to database, return to ScheduleDetailsActivity
####################################################################################################*/
    public void saveTimetableButton(View view)
    {
        ...
        for(int i=0;i<7;i++)
        {
            for(int j=0;j<timeList[i].size();j++) {
                ...
                new_schedule.getScheduleDates().add(scheduleDate);
            }
        }
        ...
        intent.putExtra(SCHEDULE_DETAILS, scheduleTemp);
...
# Inputs: None
# Return: Activity
# Return address of ModifyScheduleDatesActivity
####################################################################################################*/
