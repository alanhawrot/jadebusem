...
def OneInterchange(start_points, exclude_list, start_address, end_address, list_of_tabs, error, route_list):
    ...
    for points in start_points_id:
        ...
        for inter in interchanges_stops_id:
            isGoodDirectionFrom = interchanges_stops.filter(schedule_id=inter.schedule_id).filter(
                address__in=list_of_stops).order_by('id')[:1]
            isGoodDirectionTo = interchanges_stops.filter(schedule_id=inter.schedule_id).filter(
                address__iexact=end_address)[:1]
...
def TwoInterchanges(start_points, exclude_list, start_address, end_address, list_of_tabs, error, route_list):
    ...
    for points in start_points_id:
        ...
        end_points_id = ScheduleTracePoint.objects.exclude(schedule_id__in=exclude_list).exclude(
            schedule_id__in=start_points_id).filter(address__iexact=end_address).values("schedule").distinct()
        interchange_points = ""
        id = 0
        ...
        indirect_points_id = ScheduleTracePoint.objects.exclude(schedule_id__in=exclude_list).exclude(
            schedule_id__in=start_points_id).filter(address__in=list_of_stops).values('schedule').distinct()
        indirect_points_id = ScheduleTracePoint.objects.filter(schedule_id__in=indirect_points_id).filter(
            address__in=list_of_end_stops).values('schedule').distinct()
        indirect_points = ScheduleTracePoint.objects.filter(schedule_id__in=indirect_points_id)

        # Checking is bus driving in good direction
        for i in indirect_points_id:
            isGoodDirectionFrom = indirect_points.filter(schedule_id=i['schedule']).filter(
                address__in=list_of_stops).order_by('id')[:1]
            isGoodDirectionTo = indirect_points.filter(schedule_id=i['schedule']).filter(
                address__in=list_of_end_stops).order_by('id')[:1]
            for j in isGoodDirectionFrom:
                for k in isGoodDirectionTo:
                    if j.id < k.id:
                        last_part_id = ScheduleTracePoint.objects.exclude(schedule_id__in=exclude_list).filter(
                            address__iexact=k.address).values('schedule').distinct()
                        last_part_id = ScheduleTracePoint.objects.filter(schedule_id__in=last_part_id).filter(
                            address__iexact=end_address).values('schedule').distinct()
                        for l in last_part_id:
                            list_of_interchanges.extend(
                                [[points['schedule'], j.address, i['schedule'], k.address, l['schedule']]])
